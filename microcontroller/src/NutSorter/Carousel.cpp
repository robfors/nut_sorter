...
const float NutSorter::Carousel::_speed = 4.0; // rpm
...
NutSorter::Carousel::Carousel(StepperMotor* motor, CoterminalAngle origin_offset)
...
CoterminalAngle NutSorter::Carousel::angle()
{
  if (!_is_setup)
...
NutSorter::Slot* NutSorter::Carousel::first_slot()
{
  if (!_is_setup)
    return NULL;
...
NutSorter::Slot* NutSorter::Carousel::next_slot(Slot* slot)
{
  if (!_is_setup)
    return NULL;
